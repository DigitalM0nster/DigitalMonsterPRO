...
function measureTextWithSpacing(ctx, text, letterSpacingPx) {
	...
	if (!value || Math.abs(letterSpacingPx) < 0.001) {
		return ctx.measureText(value).width;
	}
	return Array.from(value).reduce((width, char, index) => (
		width + ctx.measureText(char).width + (index > 0 ? letterSpacingPx : 0)
	), 0);
}
...
export function getCachedArcPathTitleLayout(
	...
	ctx.font = `500 ${titleFontSize}px ${SITE_ARC_DISPLAY_FONT}`;
