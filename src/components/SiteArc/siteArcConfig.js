...
import { getGraphicsConfig, resolveRendererPixelRatio } from "@/functions/getGraphicsTier.js";

export const SITE_ARC_DISPLAY_FONT = 'ManifoldExtended, "Segoe UI", system-ui, sans-serif';
export const SITE_ARC_TEXT_COLOR = "#ffffff";

export function resolveSiteArcCanvasPixelRatio(graphicsTier = "medium") {
	const devicePixelRatio = typeof window !== "undefined" && Number.isFinite(window.devicePixelRatio)
		? window.devicePixelRatio
		: 1;
	const rendererPixelRatio = resolveRendererPixelRatio(graphicsTier, devicePixelRatio);
	const cap = getGraphicsConfig(graphicsTier).caseCanvasDprCap ?? rendererPixelRatio;
	return Math.min(rendererPixelRatio, cap);
}

/**
 * Настраиваемые параметры дуги case study — правь этот файл.
 */
export const siteArcConfig = {
...
export const siteArcRuntime = {
	 ...
	 * Пишется из siteArcFocusMotion.
	 */
	focusRotationDeg: 0,
};

/** Фиксированная геометрия и визуал — не в dev-панели. */
export const siteArcInternals = {
	/** Orbit center: places the visible right arc against the viewport edge. */
	centerXRatio: 0.65,
	centerYRatio: 0.5,
	maxNavItems: 5,
...
export function getSiteArcConfigRevision() {
	return configRevision;
}

export function subscribeSiteArcConfig(listener) {
	listeners.add(listener);
	return () => listeners.delete(listener);
}

export function bumpSiteArcConfigRevision() {
	configRevision += 1;
