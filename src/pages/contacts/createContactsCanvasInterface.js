...
function hudText(ui,id,values,{size=12,spacing=1.3,width=180,align="left",...options}={}) {
 const inkWidths=new Map();
 const item=ui.add(id,{...options,values,width,height:28,paint(ctx,value,w,h){
  ctx.font=`500 ${size}px ManifoldExtended, sans-serif`;ctx.textBaseline="middle";ctx.fillStyle="#ffffff";
...
export function createContactsCanvasInterface(renderer) {
 ...
 hudText(ui,"open",actions,{size:12,spacing:1.8,width:400,align:"center",
 ...
 for (const [i, channel] of CONTACTS_CHANNELS.entries()) {
  ...
  hudText(ui,`number${i}`,{default:String(i+1).padStart(2,"0")},{size:9,spacing:1,width:24});
 ...
 ui.layout = () => {
  ...
  ui.place("open",l.buttonX+(l.buttonWidth-400*ctaScale)/2,l.buttonY+(l.buttonHeight-28*ctaScale)/2,400*ctaScale,28*ctaScale,
  ...
  for (let i=0;i<CONTACTS_CHANNELS.length;i++) {
   ...
   ui.place(`channel${i}`,x+28,y+(l.rowHeight-28*scale)/2,180*scale,28*scale,
