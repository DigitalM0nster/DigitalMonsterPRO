 ...
 */
export function resolveSiteArcGeometry(viewportWidth, height, stateCount, isMobile, verticalBounds = null) {
	...
	const windowRotationDeg = (siteArcConfig.rotationDeg ?? 0)
 ...
 * @param {number} radius
 */
export function getSiteArcStepPositionsFromAngles(itemAngles, centerX, centerY, radius) {
 ...
 * @param {ReturnType<typeof resolveSiteArcGeometry>} geo
 * @param {number} canvasWidth
 * @param {number} canvasHeight
 */
export function drawSiteArcDebug(ctx, geo, canvasWidth, canvasHeight) {
	if (!siteArcInternals.showDebug) {
		return;
	}
	...
	const focusDeg = siteArcRuntime.focusRotationDeg ?? 0;
	ctx.fillText(
