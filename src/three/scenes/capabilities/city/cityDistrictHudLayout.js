...
export function layoutDistrictHud(width, height, anchorX, anchorY, result) {
	...
	const bottom = compact ? (height < 480 ? 62 : 92) : 24, top = height - (compact ? (height <= 480 ? 66 : 84) : 72);
	result.scale = Math.min(!compact && height < 500 ? .72 : 1, (width - left - right) / CITY_HUD_WIDTH, (top - bottom) / CITY_HUD_HEIGHT);
