...
gltf.asset.extras = { ...gltf.asset.extras, whaleParticleFormat: 'four-line-roles-surface-v2' };
const oldBinary = bytes.subarray(28 + jsonLength);
const cloud = JSON.parse(readFileSync(source, 'utf8'));
gltf.asset.extras.surfaceMetrics = cloud.surfaceMetrics;
...
encoder.SetAttributeQuantization(module.GENERIC, 14);
...
for (const [semantic, data, width, type] of [
  ...
  ['_SHELL', new Uint8Array(cloud.shell.map(value=>value*255)), 1, module.GENERIC],
]) {
  ...
  const accessor = { componentType: isJoint?5123:isByte?5121:5126, count, type: width===1?"SCALAR":`VEC${width}` };
